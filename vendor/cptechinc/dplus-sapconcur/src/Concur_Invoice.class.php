    ...
    class Concur_Invoice extends Concur_Endpoint {
        ...
        protected $structure = array(
            ...
            'detail' => array(
                ...
                'RequestLineNumber'        => array('dbcolumn' => '', 'required' => false),
        ...
        public function insert_dbinvoiceline($invnbr, $line, $debug = false) {
            $invoiceline = $this->create_dbarray($this->structure['detail'], $invoice);
            $result = false;
            
            if (does_dbinvoicelineexist($invnbr, $line['RequestLineNumber'])) {
                $result = update_dbinvoiceline($line, $debug = false);
            } else {
                $result = insert_dbinvoiceline($line, $debug = false);
            }
            return $result;
        }
        ...
        public function write_invoice($invoiceID) {
            ...
            foreach ($invoice['LineItems'] as $line) {
                $this->insert_dbinvoiceline($invoiceheader['InvoiceNumber'], $line);
            } 
        }
    }
